...
import { socialTypes } from 'constants/archetypes';
...
export default function Explorer({ title, description }) {
  ...
  const sendMessage = async () => {
    ...
    if (content.length) {
      ...
      const {
        data: { answer },
      } = await instance.post('/explorer/social', payload);
  ...
  return (
    <Box>
      <div className="explorer-header mb-10">
        <p className="font-bold text-3xl mb-2">Social Media Engine</p>
        <p className="text-xl">
          Create engaging social media content, no matter the platform.
        </p>
      </div>
